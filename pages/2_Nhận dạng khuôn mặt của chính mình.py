...
mydict = ['halo','tuhn', 'None']
vndict = ['Hồ Hà Thanh Lâm','Đỗ Duy Nhựt', 'None']
...
def camera_reg(FRAME_WINDOW, container):
    while True:
        ...
        if faces[1] is not None:
            ...
            container.text("Người trong ảnh là: " + name)
...
def main():
    st.markdown("# Nhận diện khuôn mặt của chính mình")
    st.sidebar.header("Nhận diện khuôn mặt của chính mình")
    st.sidebar.markdown("Bật camera để sử dụng chức năng này")
    container = st.empty()
